...
#define MPTCP_FALLBACK_PROXY_UNREACH	0
#define MPTCP_FALLBACK_PROXY_UNWORK	1

#define MPTCP_FALLBACK_PROXY_FAIL_THRH	5
...
struct mptcp_hw_ext_uid {
	...
	uint8_t proxy_fail; /* proxy continuous failure count, only valid if proxy is set */
...
bool mptcp_hw_ext_reset_rules_by_sk_fallback(struct sock *sk)
{
	...
	mptcp_info("%s: uid %d will fallback\n", __func__, sk->sk_uid.val);
	...
	if (node) {
		...
		kfree(node);
#ifdef CONFIG_HUAWEI_XENGINE
		xengine_report_uid_fallback(sk->sk_uid.val, MPTCP_FALLBACK_PROXY_UNREACH);
#endif
	}
	write_unlock(&mptcp_hw_ext_lock);
	return true;
...
void mptcp_proxy_chk_succ(struct sock *sk, bool succ)
{
	...
	if (succ) {
		...
		node->proxy_fail++;
		if (node->proxy_fail >= MPTCP_FALLBACK_PROXY_FAIL_THRH) {
			mptcp_info("%s: uid %d will fallback\n", __func__, uid);
			...
			kfree(node);
#ifdef CONFIG_HUAWEI_XENGINE
			xengine_report_uid_fallback(uid, MPTCP_FALLBACK_PROXY_UNWORK);
#endif
		}
	}
	write_unlock(&mptcp_hw_ext_lock);
