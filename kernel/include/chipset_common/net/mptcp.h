...
#define MPTCP_SCHED_NAME_HANDOVER "handover"
#define MPTCP_SCHED_NAME_REDUNDANT "redundant"

#define MPTCP_HANDOVER_DEFAULT_RTT_THR (200000)
...
enum mptcp_hw_cmd {
	MPTCP_HW_EXT_UID_CONF_MPTCP = 0x00000000, /* conf_value.i_val.mp_capability:
		0 or 1 */
	/* conf_value.buf.{snd_buf, rcv_buf}: valid value as per SO_SNDBUF,
	 * S_RCVBUF, vallue zero mean value old value, if both value zero mean
	 * remove config
	 */
	MPTCP_HW_EXT_UID_CONF_SO_BUF,
	MPTCP_HW_EXT_UID_CONF_SCHEDULER_TYPE,
	MPTCP_HW_EXT_UID_CONF_MPTCP_INFO,
	MPTCP_HW_EXT_UID_CONF_SCHED_PARAMS,
	MPTCP_HW_EXT_UID_CONF_PROXY_INFO,
	MPTCP_HW_EXT_UID_CONF_SUBFLOW_PRIO,

	MPTCP_HW_EXT_PID_FD_CONF_MPTCP = 0x01000000, /* conf_value.i_val.mp_capability:
		0 or 1 */
	MPTCP_HW_EXT_PID_FD_CONF_SO_BUF,
	MPTCP_HW_EXT_PID_FD_CONF_SCHEDULER_TYPE,
	MPTCP_HW_EXT_PID_FD_CONF_MPTCP_INFO,
	MPTCP_HW_EXT_PID_FD_CONF_SCHED_PARAMS,
	MPTCP_HW_EXT_PID_FD_CONF_PROXY_INFO,
	MPTCP_HW_EXT_PID_FD_CONF_SUBFLOW_PRIO,

	MPTCP_HW_EXT_DIP_DPORT_CONF_MPTCP = 0x02000000, /* conf_value.
		i_val.mp_capability: 0 or 1 */
	/* conf_value.buf.{snd_buf, rcv_buf}: valid value as per SO_SNDBUF,
	 * S_RCVBUF, value zero means old value, if both value zero mean
	 * remove config
	 */
	MPTCP_HW_EXT_DIP_DPORT_CONF_SO_BUF,
	MPTCP_HW_EXT_DIP_DPORT_CONF_SCHEDULER_TYPE,

	/* Same as MPTCP_INFO option, works based on pid and fd, currently supports
	 * getsockopt
	 */
	MPTCP_HW_EXT_DIP_DPORT_CONF_MPTCP_INFO,
	MPTCP_HW_EXT_DIP_DPORT_CONF_SCHED_PARAMS,
	MPTCP_HW_EXT_DIP_DPORT_CONF_PROXY_INFO,
	MPTCP_HW_EXT_DIP_DPORT_CONF_SUBFLOW_PRIO,

	MPTCP_HW_EXT_RESET_RULES = 0x03000000,

	MPTCP_HW_EXT_SET_CARRIER_INFO = 0x04000000,

	MPTCP_HW_EXT_RESET_CARRIER_INFO = 0x05000000,
...
/*carrier info*/
#define MPTCP_HW_EXT_CARRIER_CMCC 0
#define MPTCP_HW_EXT_CARRIER_CUCC 1
#define MPTCP_HW_EXT_CARRIER_CTCC 2
#define MPTCP_HW_EXT_CARRIER_HK 3
#define MPTCP_HW_EXT_CARRIER_OTHER 4
#define MPTCP_HW_EXT_CARRIER_INVALID 5

/* Deployment info */
#define MPTCP_HW_EXT_DEPLOY_LOADBALANCE	0
#define MPTCP_HW_EXT_DEPLOY_MASTER	1
#define MPTCP_HW_EXT_DEPLOY_SLAVE	2
#define MPTCP_HW_EXT_DEPLOY_INVALID	3

/* gid info */
#define MPTCP_HW_EXT_MAX_GID	4
...
struct mptcp_hw_proxy_info_conf
{
	__be32 proxy_ip;
	__be16 proxy_port;
};

struct mptcp_hw_ext_first_path {
	uint32_t timeout;
...
union mptcp_hw_ext_value {
	struct mptcp_hw_ext_info i_val;
	struct {
		__s32 snd_buf;
		__s32 rcv_buf;
	} buf;
	struct mptcp_hw_ext_subflow_info mp_info; /* mptcp related information */
	char sched_name[MPTCP_SCHED_NAME_MAX];
	__u32 sched_params;
	struct mptcp_hw_proxy_info_conf proxy_info;
	__u32 first_path_timeout;
	struct mptcp_hw_ext_subflow_prio subflow_prio[MAX_SUPPORT_SUBFLOW_NUM];
};

struct mptcp_hw_pid_fd {
	__s32  pid;
	__s32  fd;
};

struct mptcp_hw_dport_range {
	uint16_t dport_start;
	uint16_t dport_end;
};

#define MPTCP_PORT_RANGE_NUM_MAX 16

struct mptcp_hw_dip_dport {
	__s32  uid;
	__be32  dip;
...
struct mptcp_tcp_sock {
	...
	u16	slave_sk:1,
		...
		send_mp_other_prio:1, /*send mp_prio of other subflow on this socket*/
...
struct mptcp_pm_ops {
	...
	void (*user_switch)(struct sock *meta_sk, bool on, const char *iface);
...
struct mptcp_deleted_flow_info {
...
#define MPTCP_SCHED_DATA_SIZE 24
...
#define MPTCP_PM_SIZE 608
	...
	struct list_head deleted_flow_info;
};
...
#define mptcp_info(fmt, args...)					\
	do {								\
		if (likely(sysctl_mptcp_debug))				\
			pr_err(fmt, ##args);	\
	} while (0)
...
int mptcp_hw_ext_cmd_set_sockopt(struct mptcp_hw_ext *cmd);
int mptcp_hw_ext_cmd_get_sockopt(struct mptcp_hw_ext *cmd);
...
#define MPTCP_HW_CONF_KEY_PID_FD                (0x00000001)
#define MPTCP_HW_CONF_KEY_IP_PORT               (0x00000002)
#define MPTCP_HW_CONF_KEY_RESET                 (0x00000003)
#define MPTCP_HW_CONF_KEY_CARRIER_INFO_SET      (0x00000004)
...
#define MPTCP_HW_CONF_VALUE_SND_RCV_BUF         (1 << 1)
#define MPTCP_HW_CONF_VALUE_SCHEDULER           (1 << 2)
#define MPTCP_HW_CONF_VALUE_MPTCP_INFO          (1 << 3)
#define MPTCP_HW_CONF_VALUE_SCHED_PARAMS        (1 << 4)
#define MPTCP_HW_CONF_VALUE_PROXY_INFO          (1 << 5)
#define MPTCP_HW_CONF_VALUE_SUBFLOW_PRIO        (1 << 6)

#define mptcp_hw_ext_get_conf_type(cmd_input, conf_type)\
{\
	(conf_type) = (1 << ((cmd_input) & 0xFFFFFF));\
}

struct mptcp_hw_ext_conf_value {
	uint32_t sched_params;
	struct mptcp_hw_ext_sched {
		char sched_name[MPTCP_SCHED_NAME_MAX];
		char if_name[IFNAMSIZ];
	} sched;
	struct mptcp_hw_proxy_info_conf proxy_info;
...
void mptcp_proxy_chk_succ(struct sock *sk, bool succ);
...
static inline bool mptcp_fallback_infinite(struct sock *sk, int flag)
{
	...
	mptcp_debug("%s %#x will fallback - pi %d, src %pI4:%u dst %pI4:%u rcv_nxt %u from %pS\n",
...
static inline void mptcp_remove_shortcuts(const struct mptcp_cb *mpcb,
					  const struct sk_buff *skb) {}
static inline void mptcp_init_sub_sock(struct sock *sk, bool master_sk) {}
