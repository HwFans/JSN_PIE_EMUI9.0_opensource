...
/*{*/																									     \
	...
	SOC_SINGLE("ANA_EAR_PGA_MUTE",		CODEC_ANA_RW06_REG, 		CODEC_EAR_MUTE_OFFSET,           0x1, 0),\
	SOC_SINGLE("ANA_EAR_PGA", 			CODEC_ANA_RW26_REG, 		CODEC_EAR_GAIN_OFFSET,           0xF, 0),\
	SOC_SINGLE("ANA_LINEOUT_PGA_MUTE",	CODEC_ANA_RW06_REG, 		CODEC_LOUT_MUTE_OFFSET,          0x1, 0),\
	SOC_SINGLE("ANA_LINEOUT_PGA", 		CODEC_ANA_RW27_REG, 		CODEC_LOUT_GAIN_OFFSET,          0xF, 0),\
	SOC_SINGLE("ANA_MAINMIC_PGA_MUTE",	CODEC_ANA_RW02_REG, 		CODEC_MAINPGA_MUTE_OFFSET,       0x1, 0),\
	SOC_SINGLE("ANA_MAINPGA_BOOST",	    CODEC_ANA_RW09_REG, 		CODEC_MAINPGA_BOOST_OFFSET,      0x1, 0),\
	SOC_SINGLE_TLV("ANA_MAINMIC_PGA", 	CODEC_ANA_RW09_REG, CODEC_MAINPGA_GAIN_OFFSET, 0x7, 0, hi6555v2_ana_mainpga_gain_tlv),\
	SOC_SINGLE("ANA_AUXMIC_PGA_MUTE", 	CODEC_ANA_RW02_REG, 		CODEC_AUXPGA_MUTE_OFFSET,        0x1, 0),\
	...
	SOC_SINGLE("ANA_MIC3_PGA_MUTE", 	CODEC_ANA_RW02_REG, 		CODEC_MIC3PGA_MUTE_OFFSET,       0x1, 0),\
...
static void hi6555v2_power_codec(struct snd_soc_codec *codec, int on)
{
	...
	if (on) {
		hi6555v2_reg_write(codec, CODEC_ANA_RW03_REG, 0x77);
		hi6555v2_reg_write(codec, CODEC_ANA_RW22_REG, 0x05);
		hi6555v2_reg_write(codec, CODEC_ANA_RW23_REG, 0x3c);
		hi6555v2_reg_write(codec, CODEC_ANA_RW01_REG, 0x07);
		msleep(6);
		hi6555v2_reg_write(codec, CODEC_ANA_RW01_REG, 0x03);
		hi6555v2_reg_write(codec, CODEC_ANA_RW01_REG, 0x01);
	} else {
		hi6555v2_reg_write(codec, CODEC_ANA_RW01_REG, 0x04);
	}
...
void hi6555v2_ibias_hsmicbias_enable(struct snd_soc_codec *codec, bool enable)
{
	struct hi6555v2_priv *priv = snd_soc_codec_get_drvdata(codec);
...
static void hi6555v2_ibias_work_enable(struct snd_soc_codec *codec, bool enable)
{
	...
	if (enable) {
		...
		BUG_ON(MAX_INT32 == priv->ibias_work);
		++priv->ibias_work;
	} else {
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		hi6555v2_reg_update(CODEC_ANA_RW07_REG, MICBISA1_SET_MASK, 0x0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		hi6555v2_reg_update(CODEC_ANA_RW07_REG, MICBISA1_SET_MASK, 0xff);
		break;
	default:
		HI6555V2_LOGE("power mode event err : %d\n", event);
		break;
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		hi6555v2_reg_update(CODEC_ANA_RW07_REG, MICBISA2_SET_MASK, 0x0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		hi6555v2_reg_update(CODEC_ANA_RW07_REG, MICBISA2_SET_MASK, 0xff);
		break;
	default:
		HI6555V2_LOGE("power mode event err : %d\n", event);
		break;
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		val_mic_mux = hi6555v2_reg_read(codec, CODEC_ANA_RW09_REG);
		val_mic_mux = val_mic_mux & MAINPGA_MIC_IN_MASK;
		val_pga = hi6555v2_reg_read(codec, CODEC_ANA_RW02_REG);
		val_pga = val_pga & (0x1 << CODEC_MAINPGA_PD_OFFSET);

		/*main PGA input select HP MIC*/
		hi6555v2_reg_update(CODEC_ANA_RW09_REG, MAINPGA_MIC_IN_MASK, MAINPGA_MIC_IN_HP);

		/*close MAIN PGA*/
		hi6555v2_set_reg_bits(CODEC_ANA_RW02_REG, 0x1 << CODEC_MAINPGA_PD_OFFSET);
		msleep(1);

		/*open MAIN PGA*/
		hi6555v2_clr_reg_bits(CODEC_ANA_RW02_REG, 0x1 << CODEC_MAINPGA_PD_OFFSET);
		msleep(20);
		/*close MAIN PGA*/
		hi6555v2_set_reg_bits(CODEC_ANA_RW02_REG, 0x1 << CODEC_MAINPGA_PD_OFFSET);
		msleep(1);
		...
		hi6555v2_reg_update(CODEC_ANA_RW09_REG, MAINPGA_MIC_IN_MASK, val_mic_mux);
		hi6555v2_reg_update(CODEC_ANA_RW02_REG, (0x1 << CODEC_MAINPGA_PD_OFFSET), val_pga);
		msleep(1);
...
static const struct snd_kcontrol_new hi6555v2_ana_rcv_switch_dapm_controls[] = {
	SOC_DAPM_SINGLE("ENABLE",
			CODEC_ANA_RW06_REG, CODEC_EAR_PD_OFFSET, 1, 1/* INVERT */),
};

static const struct snd_kcontrol_new hi6555v2_ana_lout_switch_dapm_controls[] = {
	SOC_DAPM_SINGLE("ENABLE",
			CODEC_ANA_RW06_REG, CODEC_LOUT_PD_OFFSET, 1, 1/* INVERT */),
};
...
SOC_ENUM_SINGLE(CODEC_ANA_RW09_REG, CODEC_MAINPGA_SEL_OFFSET,
...
static void _hi6555v2_hsmicbias_force_close(struct snd_soc_codec *codec)
{
	...
	hi6555v2_set_reg_bits(CODEC_ANA_RW08_REG, 0x1 << CODEC_MBHD_BUFF_PD_OFFSET);
	hi6555v2_set_reg_bits(CODEC_ANA_RW08_REG, 0x1 << CODEC_MBHD_COMP_PD_OFFSET);

	/* hs micbias pd */
	hi6555v2_set_reg_bits(CODEC_ANA_RW07_REG, 0x1 << CODEC_HSMIC_PD_OFFSET);
	hi6555v2_set_reg_bits(CODEC_ANA_RW07_REG, 0x1 << CODEC_HSMICB_DSCHG_OFFSET);
	msleep(5);
	hi6555v2_clr_reg_bits(CODEC_ANA_RW07_REG, 0x1 << CODEC_HSMICB_DSCHG_OFFSET);
}
...
static int _asp_clk_enable(struct snd_soc_codec *codec)
{
	...
	BUG_ON(!priv);
	BUG_ON(!priv->asp_subsys_clk);
	BUG_ON(!priv->asp_49m_clk);
